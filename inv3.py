        ...
        try:
            with open(path, 'r') as f:
                ...
                if manifest and 'metadata' in manifest and 'cluster_name' in manifest['metadata']:
                    # Vérifie si le cluster est déjà provisionné
                    provisioned = manifest.get('metadata', {}).get('provisioned', 'no')
                    # Ajoute le cluster uniquement si provisioned est 'no'
                    if provisioned != True:
        ...
        for container in containers:
            ...
            if status == "Running":
                ...
                if ip_address:
                    ...
                    # Configuration SSH et variables personnalisées
                    host_vars = {
...
def get_combined_inventory():
    """Combine les inventaires de tous les clusters non provisionnés"""
    # Récupère tous les noms de clusters non provisionnés
    cluster_names = list_cluster_names_from_manifests()

    if not cluster_names:
        print("Aucun cluster non provisionné trouvé dans les manifests", file=sys.stderr)
